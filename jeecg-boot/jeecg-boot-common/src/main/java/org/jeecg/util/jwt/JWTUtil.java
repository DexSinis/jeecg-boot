...
public class JWTUtil {
     ...
     * @return
     */
    public static JWTResult createToken(Integer consulterId, String bmark, String flag, Integer appid, long validTime) {
        ...
        return JWTResult.buildSuccess(consulterId,bmark,flag,appid, startTime, endTime, token);
    }
     ...
     * @return
     */
    public static JWTResult verifyToken(String token, long updatePasswordTime, long lastCreateTokenTime) {
     ...
     * @return
     */
    public static JWTResult verifyToken(String token, long updatePasswordTime, long lastCreateTokenTime, boolean verifyPassword, boolean verifyOldToken) {
        ...
        try {
            ...
            return JWTResult.buildError("token无效", token);
        }
        ...
        if (verifyPassword && startTime < updatePasswordTime) {
            return JWTResult.buildError("token无效,密码已被修改,需重新授权", token);//处理密码被修改后,需重新登录需求
        }
        if (verifyOldToken && startTime < lastCreateTokenTime) {
            return JWTResult.buildError("token无效,不是最新token", token);//处理新token颁发后旧token失效需求
        }
        if (endTime < new Date().getTime()) {
            return JWTResult.buildError("token过期", token);
        }
     ...
     * @return
     */
    public static JWTResult refreshToken(String token, long addValidTime) {
        JWTResult jwtResult = verifyToken(token);//挑选自己的验证方法
        ...
        Integer consulterId = jwtResult.getConsulterId();
        String bmark = jwtResult.getBmark();
        String flag = jwtResult.getFlag();
        Integer appid = jwtResult.getAppid();
        long endTime= jwtResult.getEndTime() + addValidTime;
        long startTime= jwtResult.getStartTime();

        String neWtoken = JWT.create()
                .withHeader(map)//header
                .withClaim("consulterId", consulterId)//保留原来结束时间
                .withClaim("bmark", bmark)//保留原来结束时间
                .withClaim("flag", flag)//保留原来结束时间
                .withClaim("appid", appid)//保留原来结束时间
                .withClaim("startTime",startTime)//保留原来创建时间
                .withClaim("endTime", endTime)//结束时间延长
                .sign(Algorithm.HMAC256(SECRET));//加密
        return JWTResult.buildSuccess(consulterId,bmark,flag,appid, startTime, endTime, neWtoken);
     ...
     * @return
     */
    public static JWTResult notRefreshToken(String token) {
        JWTResult jwtResult = verifyToken(token);//挑选自己的验证方法
        return jwtResult;
    }
